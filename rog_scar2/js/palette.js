...
class ColorWheel {
  ...
  update(e, cw) {
    ...
    if (typeof e.touches === "undefined") {
      x = e.offsetX - cw.inner.offsetLeft;
      y = e.offsetY - cw.inner.offsetTop;
    } else {
      x = e.touches[0].offsetX - cw.inner.offsetLeft;
      y = e.touches[0].offsetY - cw.inner.offsetTop;
    }
    this.pos = {
      x: x,
      y: y
  ...
  draw(x, y, tri) {
    ...
    if (!tri) {
      ...
      for (let i = 0; i < angs.length; i++) {
        pts.push({
          x: Math.cos(this.toRadians(this.ang + angs[i])) * this.radSmall +
            this.cx,
          y: Math.sin(this.toRadians(this.ang + angs[i])) * this.radSmall +
            this.cy,
        });
      }
      // gradient 1 = black => white
      let g1 = this.ctxB.createLinearGradient(
        pts[1].x,
        pts[1].y,
        pts[2].x,
        pts[2].y
      ...
      let g2 = this.ctxB.createLinearGradient(
        pts[0].x,
        pts[0].y,
        pts[3].x,
        pts[3].y
    ...
    $(".setcolor").css({
      "background": this.dotCol
    });
    $(".headset-left-svg, .headset-right-svg").css({
      "fill": this.dotCol
    });
  }
  setEvents() {
    ...
    this.draw(60, 20, false);
  }
}

let colorWheel = new ColorWheel(150, ".palette");