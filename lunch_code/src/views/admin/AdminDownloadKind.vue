                            ...
                            <div class='row'>
                                <div class='col-sm-12'>
                                    <table class="table table-striped table-bordered datatable dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info" style="border-collapse: collapse !important">
                                        ...
                                        <tr v-for="(list, index) in downloadKindData" :key="index">
                                            <td>{{index + 1}}</td>
                                            <td>
                                            ...
                                            <td>                                              
                                                ...
                                                <button style='margin:0 10px;' type='button' @click='deleteKind(list.serial)' class="btn btn-danger">
  ...
  methods: {
    deleteKind: function(num) {
      if (confirm("確定刪除此筆資料嗎")) {
        this.$axios
          .post("/connect/save/save-download-kind.php?id=" + num + "&delete=1")
          .then(response => {
    ...
    checkForm: function() {
      ...
      this.$axios
        .post("/connect/save/save-download-kind.php?id=add", formData, {
          headers: {
    ...
    changeKind: function(num, title, s_title) {
      ...
      this.$axios
        .post("/connect/save/save-download-kind.php?id=" + num, formData, {
          headers: {
            "Content-Type": "multipart/form-data"
          }
        })
  ...
  created() {
    ...
    this.$axios.get("/connect/get/download.php", {}).then(response => {
      if (response.data.status != "error") {
        this.downloadKindData = response.data;
      }
    });
  }
};
