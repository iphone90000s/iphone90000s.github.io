                                    ...
                                    <table class="table table-striped table-bordered datatable dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info" style="border-collapse: collapse !important">
                                        ...
                                        <tr v-for="(list, index) in productKindData" :key="index">
                                            ...
                                            <td>                                              
                                                ...
                                                <!-- <router-link :to="{name:'admin-product-kind2',params:{id:list.serial,layer1:list.title}}" style='margin:0 5px;color:#fff' class="btn btn-primary">
                                                    <i class="fa fa-edit"></i>
                                                    編輯下一層
                                                </router-link> -->
                                                <button style='margin:0 5px' @click='gotoNext(list.serial,list.title);' type='button' class="btn btn-primary">
                                                    <i class="fa fa-edit"></i>
                                                    編輯下一層
                                                </button>
  ...
  methods: {
    ...
    deleteKind: function(num) {
      if (confirm("確定刪除此筆資料嗎")) {
        this.$axios
          .post("/connect/save/save-product-kind.php?id=" + num + "&delete=1")
          .then(response => {
    ...
    checkForm: function() {
      ...
      this.$axios
        .post("/connect/save/save-product-kind.php?id=add", formData, {
          headers: {
    ...
    changeKind: function(num, title) {
      ...
      this.$axios
        .post("/connect/save/save-product-kind.php?id=" + num, formData, {
          headers: {
            "Content-Type": "multipart/form-data"
          }
        })
