                            ...
                            <div class='row'>
                                <div class='col-sm-12'>
                                    <table class="table table-striped table-bordered datatable dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info" style="border-collapse: collapse !important">
                                        ...
                                        <tr v-for="(list, index) in qaKindData" :key="index">
                                            <td>{{index + 1}}</td>
                                            <td>
  ...
  methods: {
    deleteKind: function(num) {
      if (confirm("確定刪除此筆資料嗎")) {
        this.$axios
          .post("/connect/save/save-qa-kind.php?id=" + num + "&delete=1", {
            num: num
          })
          .then(response => {
    ...
    checkForm: function() {
      ...
      this.$axios
        .post("/connect/save/save-photo-kind.php?id=add", formData, {
          headers: {
    ...
    changeKind: function(num, title, s_title) {
      ...
      this.$axios
        .post("/connect/save/save-photo-kind.php?id=" + num, formData, {
          headers: {
            "Content-Type": "multipart/form-data"
          }
        })
